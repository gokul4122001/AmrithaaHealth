...
const AmbulanceBookingScreen = ({ navigation, route }) => {
    ...
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
          query,
        )}&key=AIzaSyBcdlNrQoO3pvPrrlS_uebDkU81sY0qj3E&components=country:in`,
        {
          method: 'GET',
        },
      );
  ...
  const handleNext = async () => {
    if (!pickupLocation.trim() || pickupLocation === 'Getting location...') {
      Alert.alert(
        'Missing Information',
        'Please wait for location to load or enter pickup location manually.',
      );
      return;
    }
    ...
    try {
      ...
      if (!pickupCoords || !dropCoords) {
        Alert.alert(
          'Location Error',
          'Could not retrieve valid coordinates. Please check addresses and try again.',
        );
        setIsLoadingLocation(false);
        return;
      }
        ...
        >
          ...
          <View style={styles.questionContainer}>
            ...
            <Text style={styles.question}>
             Which ambulance type do you want ?
            </Text>
          </View>
