...
const CurrentBookingCardScreen = () => {
  ...
  const fetchBookings = async () => {
    try {
      const response = await Emergency_Booking(token, 'scheduled');
  ...
  const renderCard = ({ item }) => {
    ...
    const handleCancelBooking = () => {
      console.log('Cancel booking pressed for ID:', item.id);
      // TODO: Call cancel API here if needed
    };
          ...
          {statusLower === 'pending' ? (
            ...
            <>
              <TouchableOpacity
                ...
                onPress={handleCancelBooking}
              >
                <Text style={styles.rejectText}>Cancel Booking</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.acceptButton}
                onPress={() =>
